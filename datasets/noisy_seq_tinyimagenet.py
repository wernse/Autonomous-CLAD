...
from datasets.utils.continual_dataset import (ContinualDataset,
                                              store_masked_loaders, store_noisy_masked_loaders)
...
class MyTinyImagenet(TinyImagenet):
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'is_noise'):
            ...
            return img, target, not_aug_img, is_noise

        return img, target, not_aug_img


class NoisySequentialTinyImagenet(ContinualDataset):
    ...
    def get_data_loaders(self) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
        ...
        train, test = store_noisy_masked_loaders(train_dataset, test_dataset, self, self.args.corrupt_perc)
        return train, test

    @staticmethod
    def get_backbone():
        return resnet18(NoisySequentialTinyImagenet.N_CLASSES_PER_TASK
                        * NoisySequentialTinyImagenet.N_TASKS)
    ...
    def get_normalization_transform():
        transform = transforms.Normalize(NoisySequentialTinyImagenet.MEAN, NoisySequentialTinyImagenet.STD)
        return transform

    @staticmethod
    def get_denormalization_transform():
        transform = DeNormalize(NoisySequentialTinyImagenet.MEAN, NoisySequentialTinyImagenet.STD)
        return transform
