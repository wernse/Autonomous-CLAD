...
from datasets.class_incremental_loader import IncrementalLoader
...
class ContinualModel(nn.Module):
    ...
    def get_aux_dataset(self):
        if self.args.datasetS == 'cifar100':
            ...
            ) + 'CIFAR100', train=False, download=True, transform=transforms.Compose(
                [transforms.ToTensor(), SequentialCIFAR100.get_normalization_transform()]))
        ...
        if self.args.datasetS != 'imagenet':
            self.aux_dl = torch.utils.data.DataLoader(
                aux_dset, batch_size=self.setting.batch_size, shuffle=True, drop_last=True)
    ...
    def mini_eval(self):
        ...
        test_dl = torch.utils.data.DataLoader(
            self.aux_test_dset, batch_size=self.setting.batch_size, shuffle=False)
        model.eval()
        with torch.no_grad():
            correct = 0
            total = 0
            ...
            if self.args.datasetS == 'imagenet':
                if not os.path.exists(self.args.load_cp):
                    ...
                    if isinstance(backbone, ResNet50):
                        ...
                        raise NotImplementedError(
                            "Imagenet auto-load not supported, provide path for existing checkpoint!")
                ...
                if self.args.load_cp is None or not os.path.exists(self.args.load_cp):
                    ...
                    for e in range(self.setting.pre_epochs):
                        for i, (x, y, _) in tqdm(enumerate(self.aux_dl), desc='Pre-training epoch {}'.format(e),
                                                 leave=False, total=len(self.aux_dl)):
    ...
    def __init__(self, backbone: nn.Module, loss: nn.Module,
        ...
        if args.dataset == 'seq-tinyimg':
            self.setting = IncrementalLoader.get_setting()
        else:
