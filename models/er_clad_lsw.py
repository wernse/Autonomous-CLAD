...
class ERCLADLSW(ContinualModel):
    NAME = 'er_clad_lsw'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ERCLADLSW, self).__init__(backbone, loss, args, transform)
        dataset = get_dataset(self.args)
        self.classes = dataset.N_CLASSES_PER_TASK
        self.buffers = {
            "day": {k: Buffer(round(self.args.buffer_size / self.classes), device=self.device) for k in
                range(0, self.classes)},
            "night": {k: Buffer(round(self.args.buffer_size / self.classes), device=self.device) for k in
                range(0, self.classes)}
        }
        self.current_task = 0
    ...
    def observe(self, inputs: torch.Tensor, labels: torch.Tensor, not_aug_inputs: torch.Tensor, epoch=None, task_id_nominal=None, teacher=None, noise=None):
        domain = "day" if task_id_nominal % 2 == 0 else "night"
        other_domain = "day" if task_id_nominal % 2 == 1 else "night"
        labels = labels.long()
        self.opt.zero_grad()
        ...
        for class_id in range(0, self.classes):
            if not self.buffers[domain][class_id].is_empty():
                batch_size_buf = 1

                buf_inputs_batch, buf_labels_batch, buf_logits_batch, buf_task_labels = self.buffers[domain][class_id].get_data_old(batch_size_buf, transform=self.transform)

                buf_inputs.append(buf_inputs_batch)
                buf_labels.append(buf_labels_batch)
                buf_logits.append(buf_logits_batch)

            if not self.buffers[other_domain][class_id].is_empty():
                batch_size_buf = 7

                buf_inputs_batch, buf_labels_batch, buf_logits_batch, buf_task_labels = \
                self.buffers[other_domain][class_id].get_data_old(batch_size_buf, transform=self.transform)
        ...
        for class_id in range(0, self.classes):
            ...
            self.buffers[domain][class_id].add_data(examples=not_aug_inputs[valid_mask],
                                            labels=labels[valid_mask],
                                            logits=outputs.data[valid_mask],
                                            task_labels=torch.full((len(not_aug_inputs[valid_mask]),), task_id_nominal))
        # else:
        #     print("-----SKIPPING---", task_id_nominal)
        #     for class_id in range(0, self.classes):
        #         print(class_id, self.buffers[class_id].task_labels)
        return loss.item(), 0, 0, 0, 0


# Observations
# Gradcam,
# Some of them are getting certain classes,
# Where things get right and wrong
# Usecase 0> This is what we observe from this data,
# Other ways to adapt the case study
# SATCH -> size of the assistant teacher
