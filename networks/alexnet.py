...
class STLAlexNet(nn.Module):
    def __init__(self, taskcla):
        ...
        self.last = torch.nn.Linear(2048, taskcla, bias=False)
...
class Autoencoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 16, 3, padding=1),  # 32x32x16
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # 16x16x16
            nn.Conv2d(16, 32, 3, padding=1),  # 16x16x32
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # 8x8x32
            nn.Conv2d(32, 64, 3, padding=1),  # 8x8x64
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # 4x4x64
            nn.Flatten(),
            nn.Linear(4 * 4 * 64, latent_dim)
        )
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, 4 * 4 * 64),
            nn.Unflatten(1, (64, 4, 4)),
            nn.ConvTranspose2d(64, 32, 2, stride=2),  # 8x8x32
            nn.ReLU(),
            nn.ConvTranspose2d(32, 16, 2, stride=2),  # 16x16x16
            nn.ReLU(),
            nn.ConvTranspose2d(16, 3, 2, stride=2),  # 32x32x3
            nn.Sigmoid()
        )

class AuxModel_AutoEncoder(nn.Module):
    def __init__(self, taskcla):
        super(AuxModel_AutoEncoder, self).__init__()
        self.last_ood = nn.ModuleList()
        for t, n in taskcla:
            self.last_ood.append(Autoencoder(latent_dim = 512))


    def forward(self, x, task_id):
        encoded = self.last_ood[task_id].encoder(x)
        decoded = self.last_ood[task_id].decoder(encoded)
        return decoded


class AuxModel_TaskID(nn.Module):
    def __init__(self, taskcla):
        super(AuxModel_TaskID, self).__init__()
        self.last_ood = nn.ModuleList()
        for t, n in taskcla:
            self.last_ood.append(TaskIDPredictor(hidden_size=2048))

    def forward(self, x, task_id):
        x = self.last_ood[task_id](x)
        return x
...
class SubnetAlexNet_NN_aux_test(nn.Module):
    def __init__(self, taskcla, sparsity=0.5):
        super(SubnetAlexNet_NN_aux_test, self).__init__()
...
class TaskIDPredictor(nn.Module):
    def __init__(self, hidden_size, n_tasks):
        super(TaskIDPredictor, self).__init__()
        ...
        self.last = nn.Linear(hidden_size, n_tasks, bias=False)

    def forward(self, x):
        x = self.fc1(x)
        x = self.drop1(self.relu(self.bn1(x)))

        x = self.fc2(x)
        x = self.drop1(self.relu(self.bn2(x)))

        x = self.last(x)
        return x


class SubnetAlexNet_NN_aux(nn.Module):
    def __init__(self, taskcla, sparsity=0.5):
        super(SubnetAlexNet_NN_aux, self).__init__()
        ...
        self.aux = TaskIDPredictor(2048, 100)
    ...
    def forward(self, x, task_id, mask=None, mode="train", training_masks=None, joint=False, consolidated_masks={}, weight_overlap=None, raw=False):
        ...
        if raw:
            x = self.conv1(x, weight_mask=mask['conv1.weight'], bias_mask=mask['conv1.bias'], mode=mode,
                           consolidated_masks=consolidated_masks.get('conv1.weight'), weight_overlap=weight_overlap, raw=True)

            x = self.maxpool(self.drop1(self.relu(self.bn1(x))))

            x = self.conv2(x, weight_mask=mask['conv2.weight'], bias_mask=mask['conv2.bias'], mode=mode,
                           consolidated_masks=consolidated_masks.get('conv2.weight'), weight_overlap=weight_overlap, raw=True)
            x = self.maxpool(self.drop1(self.relu(self.bn2(x))))

            x = self.conv3(x, weight_mask=mask['conv3.weight'], bias_mask=mask['conv3.bias'], mode=mode,
                           consolidated_masks=consolidated_masks.get('conv3.weight'), weight_overlap=weight_overlap, raw=True)
            x = self.maxpool(self.drop2(self.relu(self.bn3(x))))

            x = x.view(bsz, -1)
            x = self.fc1(x, weight_mask=mask['fc1.weight'], bias_mask=mask['fc1.bias'], mode=mode,
                         consolidated_masks=consolidated_masks.get('fc1.weight'), weight_overlap=weight_overlap, raw=True)
            x = self.drop2(self.relu(self.bn4(x)))

            x = self.fc2(x, weight_mask=mask['fc2.weight'], bias_mask=mask['fc2.bias'], mode=mode,
                         consolidated_masks=consolidated_masks.get('fc2.weight'), weight_overlap=weight_overlap, raw=True)
            x = self.drop2(self.relu(self.bn5(x)))
            y = None
